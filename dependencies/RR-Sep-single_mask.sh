...
if [ ! -f finalRR ]; then
	...
	gmt grdsample $bathy $box -GfinalRR/trim.t.grd -V # the area of interest
	
	#gmt grdmath finalRR/trim.t.grd MASK.grd MUL = finalRR/trim.t.grd
	...
	#while [ $width -lt $maxW ];
	do
		echo "W = $width km final filtering"
		gmt dimfilter $bathy $box -G./finalRR/${width}.dim.grd -F${dim_filter}${width} -D${dim_dist} -N${dim_quantity}${dim_sectors} # DiM filter
		
		gmt grdfilter ./finalRR/${width}.dim.grd $box -G./finalRR/dim.${width}.grd -F${dim_smooth_type}${dim_smooth_width} -D${dim_dist} # smoothing - TM added $box


		gmt grdmath ./finalRR/trim.t.grd ./finalRR/dim.${width}.grd SUB = ./finalRR/resid.grd # residual from DiM
		#gmt grdvolume ./finalRR/resid.grd -Sk -C$level -Vl | awk '{print r,$1,$2,$3,$4}' r=${width} >> $ors  # ORS from DiM
		#gmt grdvolume ./tmp/$$.sd.grd -Sk -C0/200/$level -T -Vl #| awk '{print r,$2,$3,$4}' r=${width} >> $ors  # ORS from DiM
        gmt grdmath ./finalRR/trim.t.grd ./finalRR/resid.grd SUB = ./finalRR/regional.grd # regional from DiM
